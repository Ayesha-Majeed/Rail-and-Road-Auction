...
def strict_extraction_regex(raw_text: str) -> list[str]:
    ...
    for match_obj in re.finditer(full_pattern, raw_text, re.IGNORECASE):
        ...
        # USER REQUEST: Prevent dummy/template ISBNs like 0123456789
        if "0123456789" in clean or clean == "1234567890":
            continue
...
def fetch_metadata_local_ai(image_path: str, accumulated_text: str = "", log_fn=print) -> dict:
    ...
    prompt = (
        ...
        "CRITICAL: Output MUST be perfectly valid JSON. No markdown, no trailing commas. All keys and string values MUST use double quotes.\n"
        "Required Format:\n"
        "{\n"
        "  \"title\": \"...\",\n"
        "  \"authors\": \"...\",\n"
...
def process_book(book_id: str, files: list[str], log_fn=print) -> dict:
    ...
    if official_isbn:
        ...
        if meta_api:
            ...
            api_t = meta_api.get("title", "").strip().lower()
            ai_t = meta_ai.get("title", "").strip().lower()
            
            if ai_t == "n/a" or not ai_t:
                log_fn("  ⚠️ VLM failed to extract title. Trusting API.")
                meta = meta_api
            else:
                score1 = fuzz.ratio(api_t, ai_t)
                score2 = fuzz.partial_ratio(api_t, ai_t)
                match_score = max(score1, score2)
                
                log_fn("  " + "═"*60)
                log_fn("  ⚡ TITLE VERIFICATION REPORT ⚡")
                log_fn(f"  ☁️  Google API Title : '{meta_api.get('title', '')}'")
                log_fn(f"  🧠 VLM AI Title     : '{meta_ai.get('title', '')}'")
                log_fn(f"  🎯 Match Score      : {match_score}%")
                
                if match_score >= 90:
                    log_fn("  ✅ STATUS: PASSED! (API Data Verified & Accepted)")
                    log_fn("  " + "═"*60)
                    meta = meta_api
                    # Merge missing fields from AI
                    for k in ["authors", "edition", "description"]:
                        if meta.get(k, "N/A") == "N/A" and meta_ai.get(k, "N/A") != "N/A":
                            meta[k] = meta_ai[k]
                else:
                    log_fn("  ❌ STATUS: FAILED! (API Hallucinated or Wrong ISBN)")
                    log_fn("  🔄 ACTION: Rejecting API entirely. Using VLM AI data.")
                    log_fn("  " + "═"*60)
                    meta = meta_ai
        else:
