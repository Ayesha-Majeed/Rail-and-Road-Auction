...
def open_train_lot_detail(parent_app, lot_id):
    ...
    # card3.pack(fill="x", padx=16, pady=12) # Hiding Detected Traits for now
    ...
    def _update_label_wrapping(event=None):
        ...
        try:
            ...
            # We use 160px to guarantee a safe margin from the scrollbar and prevent any trimming.
            card_wp = int(max(120, rw - 160))
            
            # In CustomTkinter, updating wraplength often requires re-setting the text to force a height recalculation
            title_val_lbl.configure(wraplength=card_wp, text=title_val_lbl.cget("text"))
            desc_val_lbl.configure(wraplength=card_wp, text=desc_val_lbl.cget("text"))
            lbl_local_parts.configure(wraplength=int(max(100, card_wp - 36)), text=lbl_local_parts.cget("text"))
        except Exception as e:
            print(f"Warning: Label wrapping failed: {e}")
    ...
    def _render_main():
        ...
        def _update_ui(image_obj=None, error_msg=None):
            ...
            res = results.get(p, {})
            if res:
                top_5 = res.get('top_5_railroads', [res.get('railroad', '-')])
                if len(top_5) > 1:
                    rr_text = "\n".join([f"{i+1}. {r}" for i, r in enumerate(top_5)])
                else:
                    rr_text = top_5[0] if top_5 else "-"
                lbl_local_railroad.configure(text=rr_text, justify="left", anchor="w")
        ...
        def _worker():
            ...
            try:
                ...
                if res and "boxes" in res:
                    ...
                    boxes = res["boxes"]
                    # Calculate dynamic scaling based on image size to prevent giant boxes on small images
                    scale_factor = max(0.5, im.width / 1500.0)
                    font_size = max(10, int(24 * scale_factor))
                    line_width = max(1, int(4 * scale_factor))
                    pad_x = max(2, int(8 * scale_factor))
                    pad_y = max(2, int(6 * scale_factor))
                    ...
                    try:
                        ...
                        try:
                            ...
                            try:
                                font = ImageFont.truetype("ubuntu-font-family/Ubuntu-B.ttf", font_size)
                            except:
                                try:
                    ...
                    def draw_nice_bbox(box, label, color):
                        return # Hiding bounding boxes for final build
                        if not box:
                            return
    ...
    def _deferred_init():
        ...
        def _wait_layout_ready(attempt=0):
            ...
            if preview_frame.winfo_width() >= 100 and preview_frame.winfo_height() >= 100:
                ...
                win.after(200, _update_label_wrapping)
                return
            if attempt < 30:
